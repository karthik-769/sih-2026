...
>>>>>>> a6456f2 (udated)
    try {
      ...
      setEvaluationError('Failed to load evaluation metrics. Please ensure reports and backend are running.');
    } finally {
  ...
  const fetchEvaluation = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch AI evaluation metrics:', err);
      setEvaluationError('Failed to load evaluation metrics. Please ensure reports are available.');
    } finally {
      setEvaluationLoading(false);
    }
  };
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-5 border-t border-slate-800 text-xs">
                <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
                  ...
                  <span className="text-base font-bold font-mono text-white">
                    {evaluationData.evaluated_records_count} Reports
                  </span>
=======
          ...
          <p className="text-xs text-slate-500 mt-1">
            Quantitative validation and benchmarking of the deterministic safety NLP pipeline against ground-truth incident records.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/20 border border-indigo-400/30 text-indigo-200">
              ...
              <span>Deterministic NLP Pipeline Verification & Performance Benchmarking</span>
            </div>
            <h2 className="text-xl font-bold tracking-tight">
              Safety Intelligence Model Performance
            </h2>
            <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
              Evaluates AI inferences against ground-truth safety observations across SIF precursor classification, IOGP Life-Saving Rules mapping, and barrier intelligence.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-5 border-t border-slate-800 text-xs">
            ...
            <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
              ...
              <span className="text-base font-bold font-mono text-indigo-300">
                v{evaluationData.pipeline_version} ({evaluationData.model_name})
              </span>
            </div>
            ...
            <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60 col-span-2">
              ...
              <span className="text-xs font-semibold text-emerald-300 truncate block">
                {evaluationData.embedding_provider}
              </span>
            </div>
          </div>
        )}
