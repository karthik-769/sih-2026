...
from app.ai.interfaces import BaseSifDetector, PreprocessedText, SifDetectionResult


class SifDetectionService(BaseSifDetector):
    """
    Serious Injury and Fatality (SIF) Precursor Detection Engine.
    Evaluates confluence of High-Energy Hazards, Line-of-Fire Worker Exposure,
    and Critical Control Breakdown to classify SIF potential.
    """
    ...
    def detect_sif(
        ...
        # Safe observation check -> Early exit if genuinely safe
        ...
        # Check for Missing Critical Controls as an explicit SIF category
        if control_failures and any(f.get("severity") in ["CRITICAL", "HIGH"] for f in control_failures):
            if "Missing Critical Controls" not in sif_categories:
                sif_categories.append("Missing Critical Controls")

        # Evaluate SIF Precursor boolean flag
        # True if we have High-energy hazard AND (direct exposure OR critical control failure)
        ...
        # Don't flag SIF for benign slip/trip or safe actions
        ...
        if not sif_precursor:
            ...
            fatality_potential = any(p.get("fatality_potential", False) for p in precursors)
            if exposure_rating == "EXTREME" and has_critical_failure:
                sif_level = "CRITICAL"
                confidence = 0.96
            elif has_critical_failure or exposure_rating == "HIGH":
                sif_level = "CRITICAL" if any(p["energy_source"] in ["ATMOSPHERIC_ASPHYXIATION_TOXIC", "HIGH_VOLTAGE_ARC_FLASH", "GRAVITATIONAL_POTENTIAL_FALL"] for p in precursors) else "HIGH"
                confidence = 0.93
            else:
                sif_level = "MEDIUM"
                confidence = 0.85
